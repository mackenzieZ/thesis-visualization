...
def predict():
    ...
    costs_array = [float(active_cost_a),float(passive_cost_a),float(defend_cost_d),float(no_cost_d)]
    ...
    effort_array=[float(active_effort_a),float(passive_effort_a),float(defend_effort_d),float(no_effort_d)]
    ...
    defender_utility = [float(d11),float(d12),float(a21),float(a22)]
    ...
    prob_success_array=[]
    effort_array_a = [effort_array[0], effort_array[1]]
    effort_array_d = [effort_array[2], effort_array[3]]
    ...
    for x in range(len(defender_utility)):
        print(x)
        if (x % 2) == 0:
            val = calc_defender_payoff(prob_success_array[x], defender_utility[x], calc_cost_array[2])
    ...
    for x in range(len(attacker_utility)):
        ...
        if (x <2):
            ...
            val = calc_attacker_payoff(prob_success_array[x], attacker_utility[x], calc_cost_array[1])
            calc_a_payoff_array.append(val)
